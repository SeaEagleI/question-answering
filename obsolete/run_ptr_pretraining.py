...
model_path_tplt = os.path.join(weights_dir, 'ptr-{}-{:.4f}.pth')
best_model_path = os.path.join(weights_dir, 'ptr_best_model.pth')
...
def train(model, train_dataloader, epoch_id, loss_fct, optimizer):
    ...
    for step_id, batch in enumerate(train_dataloader):
        ...
        inputs = {
            "u_question": batch[1],  # shape: [batch_size, max_quest_len, hidden_dim]
            "h_passage": batch[2],  # shape: [batch_size, max_seq_len, hidden_dim]
            "batch_first": True,
        }
...
def main():
    ...
    model = PointerNetwork(config.hidden_size, config.hidden_size)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = torch.nn.DataParallel(model).to(device)
    if pretrained and os.path.exists(best_model_path):
